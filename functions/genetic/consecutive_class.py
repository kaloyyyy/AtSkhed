...
def check_consecutive(offspring_table):
        ...
        for row in sched_section_results:
            ...
            if previous_day == day:
                if time_id - previous_time_id > 2:
                    long_breaks += 1
            if time_id - previous_time_id == 1 and previous_day == day:
                consecutive_count += 1
    ...
    print(f"{offspring_table}  |  >3 hour class: {penalty} | conflict: {conflict} | long breaks: {long_breaks} |",
          )
    return conflict, penalty, long_breaks

if __name__ == '__main__':
    i = 0
    for i in range(4):
        check_consecutive(f'offspring_0_{i}')
